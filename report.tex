...
At microfluidic length scales, Reynolds number is typically $\text{Re} \ll 2000$, so laminar flow is expected. Straight, smooth streaklines and absence of mixing confirm laminar behavior.
...
\subsection{Velocity Profile (Data Required)}
\textbf{To be completed when data is available.}
...
\subsection{Velocity vs.\ Syringe Height (Data Required)}
\textbf{To be completed when data is available.}
...
U_3 \propto z_1.
\]
Thus, fluid velocity in the straight channel is expected to grow linearly with syringe height.
...
\]
This assumes Newtonian behavior.

\subsection{Measured Ratio (Data Required)}
\textbf{To be completed when data is available.}

\subsection{Flow Features}
Abrupt contractions create separated regions and recirculation; gradual transitions suppress separation and produce more ordered streaklines.
...
Theory expects nearly unchanged centerline speed if cross‐section remains constant; curvature may induce secondary Dean vortices.  
\textbf{Data Required for comparison.}
