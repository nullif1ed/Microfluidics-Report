...
To convert pixel distances into physical length scales, an image with a known physical dimension (for example, channel width) is used for calibration.  
A pixel-to-length ratio is obtained as:

\texttt{alpha = (known physical length) / (measured pixel length)}

All measured lengths are then multiplied by \texttt{alpha} to convert to physical units.
...
Velocity, computed by \texttt{v = d / t}, inherits uncertainty from both measured streak length and exposure duration.

\subsection{Uncertainty Propagation of Velocity}
Given \texttt{v = d / t}, the relative uncertainty is:

\texttt{(Δv / v)^2 = (Δd / d)^2 + (Δt / t)^2}

Thus,

\texttt{Δv = v * sqrt((Δd / d)^2 + (Δt / t)^2)}
...
At microfluidic length scales, Reynolds number is typically much less than 2000, so laminar flow is expected. As seen in the captured images from the microfluidics lab, the smooth streaklines confirm laminar flow behavior in the straight tube section.
...
For pressure-driven flow, maximum velocity occurs at the channel centerline; the velocity gradient is largest near walls where no-slip applies.  
This matches theoretical parabolic Poiseuille flow.  
Adverse pressure gradient effects are not expected to play major roles in a straight microchannel.
...
Hagen--Poiseuille law gives:

\texttt{Q proportional to (Δp) / (μL), and Q = S * U}

Since \texttt{Δp = ρ * g * z1},  
we have \texttt{U3 proportional to z1.}
...
Torricelli’s law gives \texttt{U ≈ sqrt(2 * g * Δz).}  
Thus, \texttt{U3 proportional to sqrt(z1).}

Comparing:
\begin{itemize}
  \item Hagen--Poiseuille: \texttt{U3 proportional to z1}
  \item Bernoulli: \texttt{U3 proportional to sqrt(z1)}
\end{itemize}
...
\texttt{U proportional to 1 / A}

For a fixed-height channel, area ratio approximates width ratio:

\texttt{U2 / U1 ≈ W1 / W2}

This assumes Newtonian behavior. Thus, with a starting width of 241.197 micrometers and an expansion width of 507.619 micrometers, the theoretical centerline flow velocity ratio is approximately 2.105.

\subsection{Measured Ratio (Data Required)}
Before the expansion, the measured velocity was 1005.823 micrometers per second; after the expansion, it was measured at 408.012 micrometers per second. This yields a velocity ratio of 2.465, which generally agrees with the theoretically calculated value.

\subsection{Flow Features}
Abrupt contractions create separated regions and recirculation; gradual transitions suppress separation and produce more ordered streaklines. In the abrupt transition, there are pockets of turbulent or circular flow at the edges of the expansion chamber. The differences can be clearly seen in the two following figures.
...
Given Reynolds number much less than 2000, turbulent flow is unlikely; streaklines should remain smooth. Localized disturbances do not constitute sustained turbulence.
...
Theory expects nearly unchanged centerline speed if cross-section remains constant. Curvature may induce secondary Dean vortices, resulting in flow velocity discrepancy. For the sharp bend channel, velocity before the bend was measured at 714.988 micrometers per second and after the bend at 528.127 micrometers per second. For the curved bend channel, centerline velocities were measured at 560.019 micrometers per second and 694.1 micrometers per second before and after the bend, respectively.

\subsection{Flow Features}
Smooth bend results in smooth pathlines, while a sharp bend introduces recirculation, separation, and secondary vortices.  
...
\subsection{Turbulence}
Flow likely remains laminar due to low Reynolds number. Observed waviness arises from curvature, not turbulence.
